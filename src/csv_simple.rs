use std::{fs::read_to_string, mem::take, path::PathBuf};
...
impl Csv {
	pub fn parse_file(csv_file: PathBuf) -> Result<Vec<Vec<String>>, CsvParseError> {
		read_to_string(csv_file).map_or(Err(CsvParseError::UnableToOpenFile), |content| Self::parse(&content))
	}

	fn parse(content: &str) -> Result<Vec<Vec<String>>, CsvParseError> {
		let mut csv = Vec::new();
		let mut row = Vec::new();
...
mod tests {
	...
	fn parse_test_cr_only_line_endings() {
		assert_eq!(Csv::parse("a,b\rc,d"), Ok(vec![row(&["a", "b"]), row(&["c", "d"])]));
	}
	...
	fn parse_test_multiple_pending_empty_rows() {
		assert_eq!(Csv::parse("a\n\n\n\nb"), Ok(vec![row(&["a"]), row(&[""]), row(&[""]), row(&[""]), row(&["b"])]));
	}
	...
	fn parse_test_trailing_comma_is_empty_field() {
		assert_eq!(Csv::parse("a,b,"), Ok(vec![row(&["a", "b", ""])]));
	}
	...
	fn parse_test_lone_comma() {
		assert_eq!(Csv::parse(","), Ok(vec![row(&["", ""])]));
	}
	...
	fn parse_test_blank_line_between_rows() {
		...
		assert_eq!(Csv::parse("a,b,c\n\n4,5,6"), Ok(vec![row(&["a", "b", "c"]), row(&[""]), row(&["4", "5", "6"])]));
	}
